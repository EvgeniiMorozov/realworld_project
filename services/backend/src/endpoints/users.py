from core.auth import clear_token, encode_jwt
...
@users_router.post(
    "/users/login", response_model=UserResponce, tags=["User and Authentication"]
)
async def authentication(
    user_login: LoginUserRequest, db: AsyncSession = Depends(get_session)
) -> UserResponce:
...
async def register_user(
    new_user: NewUserRequest, db: AsyncSession = Depends(get_session)
) -> UserResponce:
    """Register a new user."""
    ...
    if db_user:
        raise HTTPException(
            ...
            detail=f"Email {new_user.user.email} already registered",
        )
...
@users_router.get(
    "/user", response_model=UserResponce, tags=["User and Authentication"]
)
...
@users_router.put(
    "/user", response_model=UpdateUserRequest, tags=["User and Authentication"]
)
...
@users_router.get(
    "/profiles/{username}",
    response_model=ProfileUserResponce,
    tags=["User and Authentication"],
)
async def get_profile(
    request: Request, username: str, db: AsyncSession = Depends(get_session)
) -> ProfileUserResponce:
...
@users_router.post(
    "/profiles/{username}/follow", response_model=ProfileUserResponce, tags=["Profile"]
)
async def create_follow(
    ...
    if follower == following:
        raise HTTPException(
            status_code=HTTP_403_FORBIDDEN, detail="You cannot subscribe to yourself"
        )

    subscribe = users_crud.check_subscribe(db, follower.username, following.username)
    if subscribe:
        raise HTTPException(
            status_code=HTTP_403_FORBIDDEN, detail="You are already a subscribed user"
        )

    users_crud.create_subscribe(
...
async def delete_follow(
    ...
    if not subscribe:
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST,
            detail=f"You are not a subscribed this user '{username}'",
        )

    users_crud.delete_subscribe(
        db, user_username=follower.username, author_username=following.username
    )
    return ProfileUserResponce(profile=following)
